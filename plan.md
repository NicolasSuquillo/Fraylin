...
- **Fase 0**: ✅ resuelto. Conectividad OK con `DATABASE_URL` pooled (`aws-1-us-east-2.pooler.supabase.com:6543`). Sin `DIRECT_URL` (no necesario).
- **Fase 1**: ✅ completa. `db:generate`, `db:migrate`, `db:seed` corrieron OK contra Supabase. 8 categorías, 13 productos, 13 imágenes, 10 galería.
- **Fase 2**: ✅ completa y verificada (build, tsc, dev en vivo: home, login, CRUD productos/categorías/galería, 409s, 401s, revalidatePath). Cambios sin commit en working tree (lib/products, lib/gallery, types, todas las API admin, ProductForm, page.tsx, money.ts, seed.ts).
- **Fase 3**: ✅ completa y verificada. `@vercel/blob` instalado; `api/admin/upload/route.ts` usa `put()`; `delete-public-upload.ts` usa `del()` para URLs Blob y es no-op en producción para rutas legacy `/products|/gallery` (solo borra en dev); `next.config.ts` con `images.remotePatterns` para `*.public.blob.vercel-storage.com`. `BLOB_READ_WRITE_TOKEN`/`BLOB_STORE_ID` en `.env.local`. Probado: upload real devuelve URL Blob, `next/image` la optimiza (200), `del()` la elimina.
- Nota: durante Fase 1 quedó dato corrupto (encoding) en `GRI-001` por un seed parcial previo; resuelto con `db:seed -- --reset`.
- **Fase 4**: ✅ código completo (CartProvider/Drawer/Button/AddToCart, integraciones layout/Header/ProductCard/ProductModal). `npm run build` pasa; HTML SSR confirma `priceCents`/`$25.00` para producto de prueba GRI-001 (priceCents=2500, stock=5 vía `scripts/set-test-price.ts`). Verificación interactiva (drawer, badge, localStorage) pendiente de prueba manual del usuario en `localhost:3002` — sin entorno gráfico para Playwright.
- **Fase 5**: ✅ código completo y verificado parcialmente (sin credenciales Payphone reales). Investigación en docs.payphone.app: CDN `box/v2.0` (`payphone-payment-box.js/.css`), constructor `PPaymentButtonBox`, Confirm en `https://paymentbox.payphonetodoesposible.com/api/confirm` (body `{id, clientTxId}`, `statusCode` 2=Canceled/3=Approved). Implementado: `src/lib/payphone.ts`, `src/lib/orders.ts` (`finalizeOrder`/`cancelOrder` idempotentes con `db.transaction`), `src/app/api/checkout/route.ts`, `src/app/checkout/{page,CheckoutClient}.tsx`, `src/app/checkout/respuesta/{page,ClearCartOnSuccess}.tsx`. `npm run build` ✅. Probado por API: creación de orden `pending` ✅, validación de stock insuficiente ✅, producto no comprable ✅, carrito vacío ✅, redirect cancelado (`id=0`) marca orden `cancelled` sin tocar stock ✅. **Credenciales reales configuradas** (`PAYPHONE_TOKEN` modo Prueba + `NEXT_PUBLIC_SITE_URL`; `PAYPHONE_STORE_ID` vacío — el "Identificador" de la app de Payphone no es un storeId válido, `storeId` se hizo opcional en `CheckoutClient.tsx`). **Verificado visualmente**: la Cajita renderiza completa (tarjeta, "(TEST) USD 25.00", botón Pagar, "Powered by payphone"). **Pendiente**: completar un pago de prueba real (tarjeta de prueba de Payphone) para validar `finalizeOrder` end-to-end (descuento de stock, idempotencia en recarga, redirect `/checkout/respuesta`).
- **Fase 6**: ✅ completa y verificada. `src/lib/orders.ts` añade `getOrders`/`getOrderById`/`updateFulfillmentStatus` + `FULFILLMENT_STATUSES`/`FulfillmentStatus` (movidos a `src/types/index.ts` para evitar bundlear `postgres`/`db` en componentes cliente). Nuevas rutas `src/app/api/admin/orders/route.ts` (GET) y `[id]/route.ts` (GET, PATCH fulfillmentStatus con validación 400). UI: `src/app/admin/orders/page.tsx`, `OrdersTable.tsx`, `StatusBadge.tsx`, `FulfillmentSelect.tsx`, `[id]/page.tsx` (detalle con items, totales, datos cliente, IDs Payphone, botón WhatsApp al cliente). `AdminNav.tsx` con entrada "Pedidos". `npm run build` ✅. Probado en dev con datos reales: 401 sin sesión, redirect `/admin-login` en página, 200 con sesión, listado de 5 órdenes reales (incluye una `paid`), detalle 200, PATCH fulfillment 200 (revertido a `nuevo`), PATCH con valor inválido → 400.
- **Pago de prueba Payphone**: ✅ validado end-to-end por el usuario (orden `paid` en BD, stock descontado).
- **Fase 7 (limpieza, parcial — 2026-06-11)**: ✅ GRI-001 revertido a `priceCents=null, stock=null`; eliminados `scripts/set-test-price.ts`, `scripts/seed.ts`, `src/data/products.json`, `src/data/gallery.json` y el script npm `db:seed` (la BD es la única fuente de verdad; los JSON quedaron obsoletos respecto a ediciones del admin). `CLAUDE.md` actualizado con la arquitectura nueva. Nota: el paso 2 de Fase 7 (db:seed contra producción) queda obsoleto — es el mismo Supabase ya poblado.
- Pendiente: decidir cuándo commitear, deploy en Vercel (env vars, responseUrl de producción, smoke test).
